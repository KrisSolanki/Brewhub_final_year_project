...
const userId = location?.state?.user_Id;  // Extract the userId from the location state
// console.log("UserID",location.state.user_Id)
  ...
  const handleNext = async (e) => {
   ...
   const response = await axios.post("http://127.0.0.1:8000/api/register/address/", {
        Home: home,
        Street: street,
        Landmark: landmark,
        Pincode: pincode,
    ...
    console.log("Exit point",response)
    if(response.data.status == 200){
      console.log("After request body",response)
      navigate("/login") 
      //  navigate("/", { state: { formData: { home, street, landmark, pincode, city, state } } });
    } else{
      console.log("ERROR")
    }
